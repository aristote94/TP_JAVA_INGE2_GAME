...
public class Dungeon {
    ...
    private void respawnListOfThings(){
        ...
        for (int x=0;x<width;x++){
            for (int y=0;y<height;y++){
                switch (this.map[x][y]){
                    case ' ' :  renderList.add(new Things(x* tileManager.getWidth(),y* tileManager.getHeight(), tileManager.getTile(0,2)));
                                break;
                    case 'W' :  renderList.add(new SolidThings(x* tileManager.getWidth(),y* tileManager.getHeight(), tileManager.getTile(0,0)));
                                break;
                    case 'E' :  renderList.add(new SolidThings(x* tileManager.getWidth(),y* tileManager.getHeight(), tileManager.getTile(0,1)));
                                break;
                }
            }
        }
